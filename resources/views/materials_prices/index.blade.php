<div class="row">
	<div class="col-lg-12">
		<div class="card mb-3">
			<h3 class="card-header text-primary">Ціни на матеріальні ресурси
			...
			<div class="card-body">
				<div id="cardCollpase" class="collapse show">
					<div class="row mb-2">
						<div class="col-lg-12">
							<a href="/materials_prices/create" class="btn btn-primary mb-1">
								<i class="mdi mdi-database-plus"></i> Додати
							</a>
							<a href="/materials_prices/create/tabular" class="btn btn-info mb-1">
								<i class="mdi mdi-database-plus"></i> Масове додавання на {{ date('Y') + 1 }} рік
							</a>
						</div>
					</div>
					@if (count($results))
							...
							<table class="table table-centered table-hover table-bordered table-sm mb-0">
								<thead>
									<tr class="text-center">
										<th>ID</th>
										<th>Матеріальний ресурс</th>
										<th>Ціна без НДС, грн</th>
										<th>Ціна з НДС, грн</th>
										<th>Рік</th>
										{{-- <th>Запис створив</th>
								...
								<tbody>
									@foreach ($results as $item)
										<tr class="text-center">
											...
											<td>{{ $item->price }}</td>
											<td>{{ number_format($item->price * 1.2, 2) }}</td>
											<td>{{ $item->price_year }}</td>
											{{-- <td>{{ $item->created_by }}</td>
											<td>{{ $item->created_at }}</td>
											<td>{{ $item->updated_by }}</td>
											<td>{{ $item->updated_at }}</td> --}}
											<td>
												<form action="/materials_prices/{{ $item->id }}" method="POST">
													<a href="/materials_prices/{{ $item->id }}" class="btn btn-outline-info btn-sm my-1">
														<i class="mdi mdi-eye"></i>
													</a>
													<a href="/materials_prices/{{ $item->id }}/edit" class="btn btn-outline-success btn-sm my-1">
