<div class="row">
	<div class="col-lg-12">
		<div class="card mb-3">
			<h3 class="card-header text-primary">Графіки ремонтів
			...
			<div class="card-body">
				<div id="cardCollpase" class="collapse show">
					{{-- <div class="row mb-2">
						<div class="col-lg-12">
							<a href="/schedules/create" class="btn btn-primary mb-1">
							...
							<table class="table table-centered table-hover table-bordered table-sm mb-0">
								<thead>
									<tr class="text-center">
										...
										<th>Тип ремонту</th>
										<th>Періодичність, міс.</th>
										<th>Дата останього ремонту</th>
								...
								<tbody>
									@foreach ($results as $item)
										<tr class="text-center">
											...
											<td>{{ $item->specific_renovation_object_name }}</td>
											<td>{{ $item->type_service_name }}</td>
											<td>{{ $item->periodicity }}</td>
											<td>{{ $item->date_last_service }}</td>
											<td>
												@if ($item->status == 1)
													<i class="mdi mdi-check-circle-outline mdi-24px text-success"></i>
												@else
													<i class="mdi mdi-check-circle-outline mdi-24px text-danger"></i>
												@endif
											</td>
											...
											<td>
												<form action="/schedules/{{ $item->id }}" method="POST">
													<a href="/schedules/{{ $item->id }}" class="btn btn-outline-info btn-sm my-1">
														<i class="mdi mdi-eye"></i>
													</a>
													<a href="/schedules/{{ $item->id }}/edit" class="btn btn-outline-success btn-sm my-1">
